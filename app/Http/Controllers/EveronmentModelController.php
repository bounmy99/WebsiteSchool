...
use App\Models\EveronmentModel;
use Illuminate\Http\Request;

class EveronmentModelController extends Controller
{
       /**
    ...
    {
        $images = EveronmentModel::orderBy('id','desc')
        ->paginate(8)
        ->toArray();
    ...
    {
        $updateContent = EveronmentModel::find($id);
    ...
    {
        $update_logo = EveronmentModel::find($id);
    ...
    public function EditSelectedItems(Request $request){
        try{
            ...
            $images = EveronmentModel::whereIn('id', $ids)->get();
            return response()->json($images);
        }catch(Exception $e){
     ...
     */
    public function edit(EveronmentModel $EveronmentModel,$id)
    {
        try{
            $edit = EveronmentModel::find($id);
     ...
     */
    public function update(Request $request, $id, EveronmentModel $EveronmentModel)
    {
        try{
            $Update = EveronmentModel::find($id);
            if($request->file('file')){
     ...
     */
    public function destroy(EveronmentModel $EveronmentModel, Request $request)
    {
        {
            $ids = $request->input('ids');
            foreach($ids as $id){
                $delete = EveronmentModel::find($id);
                if($delete){
                    $path = "assets/img/images";
                    if($delete->images !=''){
            ...
            $img = EveronmentModel::whereIn('id', $ids)->delete();
