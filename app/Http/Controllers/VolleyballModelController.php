...
class VolleyballModelController extends Controller
{
          /**
    ...
    {
        $images = VolleyballModel::orderBy('id','desc')
        ->paginate(8)
        ->toArray();
    ...
    {
        $updateContent = VolleyballModel::find($id);
    ...
    public function EditSelectedItems(Request $request){
        try{
            ...
            $images = VolleyballModel::whereIn('id', $ids)->get();
            return response()->json($images);
        }catch(Exception $e){
     ...
     */
    public function edit(VolleyballModel $VolleyballModel,$id)
    {
        try{
            $edit = VolleyballModel::find($id);
     ...
     */
    public function update(Request $request, $id, VolleyballModel $VolleyballModel)
    {
        try{
            $Update = VolleyballModel::find($id);
            if($request->file('file')){
     ...
     */
    public function destroy(VolleyballModel $VolleyballModel, Request $request)
    {
        {
            $ids = $request->input('ids');
            foreach($ids as $id){
                $delete = VolleyballModel::find($id);
                if($delete){
                    $path = "assets/img/images";
                    if($delete->images !=''){
            ...
            $img = VolleyballModel::whereIn('id', $ids)->delete();
