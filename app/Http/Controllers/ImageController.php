...
class ImageController extends Controller
{
    ...
    {
        $images = ImageModel::orderBy('id','desc')
        ->paginate(8)
        ->toArray();
    ...
    {
        $updateContent = ImageModel::find($id);
    ...
    public function EditSelectedItems(Request $request){
        try{
            ...
            $images = ImageModel::whereIn('id', $ids)->get();
            return response()->json($images);
        }catch(Exception $e){
     ...
     */
    public function edit(ImageModel $ImageModel,$id)
    {
        try{
            $edit = ImageModel::find($id);
     ...
     */
    public function update(Request $request, $id, ImageModel $ImageModel)
    {
        try{
            $Update = ImageModel::find($id);
     ...
     */
    public function destroy(ImageModel $ImageModel, Request $request)
    {
        {
            $ids = $request->input('ids');
            foreach($ids as $id){
                $delete = ImageModel::find($id);
                if($delete){
                    $path = "assets/img/images";
                    if($delete->images !=''){
            ...
            $img = ImageModel::whereIn('id', $ids)->delete();
