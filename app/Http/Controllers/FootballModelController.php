...
class FootballModelController extends Controller
{
       /**
    ...
    {
        $images = FootballModel::orderBy('id','desc')
        ->paginate(8)
        ->toArray();
    ...
    {
        $updateContent = FootballModel::find($id);
    ...
    {
        $update_logo = FootballModel::find($id);
    ...
    public function EditSelectedItems(Request $request){
        try{
            ...
            $images = FootballModel::whereIn('id', $ids)->get();
            return response()->json($images);
        }catch(Exception $e){
     ...
     */
    public function edit(FootballModel $FootballModel,$id)
    {
        try{
            $edit = FootballModel::find($id);
     ...
     */
    public function update(Request $request, $id, FootballModel $FootballModel)
    {
        try{
            $Update = FootballModel::find($id);
            if($request->file('file')){
     ...
     */
    public function destroy(FootballModel $FootballModel, Request $request)
    {
        {
            $ids = $request->input('ids');
            foreach($ids as $id){
                $delete = FootballModel::find($id);
                if($delete){
                    $path = "assets/img/images";
                    if($delete->images !=''){
            ...
            $img = FootballModel::whereIn('id', $ids)->delete();
