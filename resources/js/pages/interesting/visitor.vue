                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-footer d-flex justify-content-center">
                                    <button type="button" class="btn btn-primary"
                                        @click="update(this.Id_img)">ອັບເດດ</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    methods: {
        ...
        ContetnUpdate(id) {
            if (this.content == '') {
                ...
                this.$axios.post(`api/visitor//update/content/${id}`, { content: this.content }).then((response) => {
                    this.GetData()
                }).catch((error) => {
                    console.log(error);
                })
        ...
        uploadImages() {
            if (this.content == '') {
                ...
                this.$axios.post('api/visitor/add', formData, { headers: { "Content-Type": "multipart/form-data" } }).then(response => {
        ...
        GetData(page) {
            this.$axios.get(`api/visitor/getAll?page=${page}`).then((response) => {
            ...
            this.$swal({
                ...
                if (result.isConfirmed) {
                    if (this.selectedItems.length > 0) {
                        this.$axios.post('api/visitor/deleteSelectedItems', { ids: this.selectedItems })
                            .then((response) => {
        ...
        editImages(id) {
            ...
            this.$axios.get('api/visitor/edit/' + id).then((response) => {
        ...
        update(id) {
            ...
            this.$axios.post(`api/visitor/update/${id}`, Data, { headers: { "content-Type": "multipart\form-data" } }).then((response) => {
                if (response.data) {
                    this.$swal({
                        position: 'center',
                        icon: 'success',
        ...
        EditSelectedItems() {
            this.FormType = true
            this.$axios.post('api/visitor/EditSelectedItems', { ids: this.selectedItems })
                .then((response) => {
