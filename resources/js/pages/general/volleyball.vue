    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <div class="card-body" v-if="isLoggin">
                <h3 class="fw-bold">ລາຍການຮູບທີ່ເລືອກທັງໝົດ {{ images.length }} ຮູບ</h3>
                <div class="mb-3" v-if="!FormType">
                    <label class="form-label fs-5 fw-bold">ພາກສ່ວນ Logo</label>
                    <div class="row">
                        <div class="col-md-10">
                            <button type="button" class="logo" @click="UploadLogo()">
                                <img :src="image_previews" alt="" class="img" />
                            </button>
                            <div class="">
                                <input type="file" class="form-control" style="display:none" ref="files"
                                    @change="onSelectedLogo">
                            </div>
                        </div>
                        <div class="col-md-2 text-end">
                            <button class="btn btn-outline-success"  @click="LogoUpdate(this.content_id)"><i class='bx bx-save me-1'></i>ບັນທຶກ</button>
                ...
                <form @submit.prevent="uploadImages" v-if="!FormType">
                    ...
                    <div class="mb-3">
                        <label class="form-label fs-5 fw-bold">ພາກສ່ວນຂຽນເນື້ອຫາຕ່າງໆ</label>
                        <div class="row">
                            <div class="col-md-10">
                    ...
                    <button type="submit" class="btn btn-primary mt-5 me-1" :disabled="disable">ອັບໂຫຼດຮູບພາບ</button>
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer d-flex justify-content-center">
                                <button type="button" class="btn btn-primary" @click="update(this.Id_img)"><i class='bx bx-sync'></i>ອັບເດດ</button>
                                <button type="button" class="btn btn-success" @click="Upload()"><i class="bx bx-camera me-1"></i>ປ່ຽນຮູບ</button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card mt-3" v-if="!FormType">
            <div class="card-body">
                <div class="row g-2 mb-5">
                    <div class="row" v-if="isLoggin">
                        ...
                        <div class="col-md-8 d-flex justify-content-end">
                            <button @click="EditSelectedItems" class="btn btn-success me-2"
                                :disabled="disableBtn_del"><i class='bx bx-edit me-1'></i>ແກ້ໄຂພາບທີ່ເລືອກ</button>
                            <button @click="deleteSelectedItems" class="btn btn-danger"
                                :disabled="disableBtn_del"><i class='bx bx-trash-alt me-1'></i>ລົບຮູບພາບທີ່ເລືອກ</button>
                        </div>
                    </div>
                    ...
                    <div v-if="!isLoggin">
                        <div class="d-flex justify-content-center mb-2  h-10">
                            <img :src="('assets/img/logo/logo/' + this.logo)" atl="" style="width:20%">
                        </div>
                    ...
                    <div class="col-md-6 col-lg-3 col-sm-3 mb-12 mb-3" v-for="itemImg in GetImage.data" :key="itemImg.id">
                        <div class="card h-100">
                            ...
                            <a :href="('assets/img/images/' + itemImg.images)">
                                <img class="card-img-top" :src="('assets/img/images/' + itemImg.images)" alt="Card image cap">
                            </a>
                        </div>
                    </div>
                </div>
...
export default {
    name: 'WorkspaceJsonvolleyballing',
    ...
    data() {
        return {
            ...
            FormType: false,
            Id_img: '',
            image_previews: window.location.origin + '/assets/img/logo/preview.JPG',
    ...
    methods: {
        ...
        onSelectedLogo(event) {
            this.img_logo = event.target.files[0];
            let reader = new FileReader();
            reader.readAsDataURL(this.img_logo);
            reader.addEventListener("load", function () {
                this.image_previews = reader.result;
            }.bind(this), false);
        },
        UploadLogo(event) {
        ...
        ContetnUpdate(id) {
            if (this.content == '') {
                ...
                this.$axios.post(`api/volleyball/update/content/${id}`, { content: this.content }).then((response) => {
                    this.GetData()
                }).catch((error) => {
                    console.log(error);
                })
        ...
        LogoUpdate(id) {
            ...
            Data.append("file", this.img_logo);
            this.$axios.post(`api/volleyball/update/logo/${id}`, Data, { headers: { "content-Type": "multipart/form-data" } }).then((response) => {
                if (response.data) {
                    this.$swal({
                        position: 'center',
                        icon: 'success',
        ...
        uploadImages() {
            if (this.content == '') {
                ...
                this.$axios.post('api/volleyball/add', formData, { headers: { "Content-Type": "multipart/form-data" } }).then(response => {
        ...
        GetData(page) {
            this.$axios.get(`api/volleyball/getAll?page=${page}`).then((response) => {
                ...
                if(response.data.data[0].logo){
                    this.image_previews = window.location.origin + '/assets/img/logo/logo/' + response.data.data[0].logo
                }else{
                    this.image_previews
                }
            }).catch((error) => {
            ...
            this.$swal({
                ...
                if (result.isConfirmed) {
                    if (this.selectedItems.length > 0) {
                        this.$axios.post('api/volleyball/deleteSelectedItems', { ids: this.selectedItems })
                            .then((response) => {
        ...
        editImages(id) {
            ...
            this.$axios.get('api/volleyball/edit/' + id).then((response) => {
        ...
        update(id) {
            ...
            this.$axios.post(`api/volleyball/update/${id}`, Data, { headers: { "content-Type": "multipart/form-data" } }).then((response) => {
                if (response.data) {
                    this.$swal({
                        position: 'center',
                        icon: 'success',
        ...
        EditSelectedItems() {
            this.FormType = true
            this.$axios.post('api/volleyball/EditSelectedItems', { ids: this.selectedItems })
                .then((response) => {
